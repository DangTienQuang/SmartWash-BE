...
namespace AutoWashPro.BLL.Services
{
    public class VoucherService : IVoucherService
    {
        ...
        public async Task RedeemVoucherAsync(int userId, int voucherId)
        {
            ...
            try
            {
                ...
                if (voucher.RequiredTierId.HasValue)
                {
                    ...
                    {
                        throw new BadRequestException($"You need to reach {voucher.RequiredTier?.TierName} tier to redeem this voucher.");
                    }
                }
            ...
            catch (DbUpdateException)
            {
                ...
                throw new BadRequestException("You already own this voucher, please do not perform actions too quickly.");
            }
            catch
            {
                await transaction.RollbackAsync();
        ...
        public async Task<bool> DeleteVoucherAsync(int id)
        {
            ...
            if (hasOwners)
                throw new BadRequestException("Cannot delete a voucher that has already been claimed by a customer. Please deactivate it or let it expire.");
        ...
        public async Task<bool> ConsumePhysicalVoucherAsync(int userId, string voucherCode)
        {
            ...
            if (userVoucher == null) throw new NotFoundException("Voucher code does not exist in the system or the customer has not claimed it.");
            if (userVoucher.Voucher.VoucherType != VoucherType.PhysicalGift)
                throw new BadRequestException("This voucher is not a physical gift type.");
            ...
            if (userVoucher.UsageCount >= userVoucher.Voucher.MaxUsagePerUser)
                throw new BadRequestException("This voucher has run out of usages.");
        ...
        private static void ValidateVoucherAvailability(Voucher voucher)
        {
            ...
            if (voucher.StartDate.HasValue && voucher.StartDate.Value > DateTime.UtcNow) throw new BadRequestException("Voucher is not yet applicable.");
            if (voucher.ExpiryDate < DateTime.UtcNow) throw new BadRequestException("Voucher has expired.");
            if (voucher.MaxUsages > 0 && voucher.CurrentUsageCount >= voucher.MaxUsages) throw new BadRequestException("Voucher has run out of usages.");
        }
