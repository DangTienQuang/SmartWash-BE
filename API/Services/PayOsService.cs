...
namespace AutoWashPro.API.Services
{
    public class PayOsService : IPayOsService
    {
        ...
        public PayOsService(IConfiguration configuration)
        {
            ...
            // Sử dụng Reflection để khởi tạo vì xung đột namespace/class trong SDK
        ...
        public async Task<PayOsPaymentResult> CreatePaymentLinkAsync(long orderCode, int amount, string description, string userId)
        {
            ...
            // Khởi tạo PaymentData: orderCode, amount, description, items, cancelUrl, returnUrl
        ...
        public async Task<PayOsWebhookResult?> VerifyWebhookDataAsync(object webhookBody)
        {
            try
            {
                // Ép kiểu body sang WebhookType của SDK
                var asm = _payOS.GetType().Assembly;
                var webhookType = asm.GetType("PayOS.Types.WebhookType");
                
                // Giả định webhookBody đã được Deserialize đúng hoặc là JsonElement
                // Trong thực tế, Controller nên Deserialize trực tiếp hoặc chúng ta dùng Reflection để map
